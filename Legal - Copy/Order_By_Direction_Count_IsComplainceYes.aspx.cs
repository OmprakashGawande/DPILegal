...
public partial class Legal_Order_By_Direction_Count_IsComplainceYes : System.Web.UI.Page
{
    DataSet dsCase = null;
    ...
    protected void BindGrid(string CaseType)
    {
        try
        {
            dsCase = obj.ByDataSet("select distinct UniqueNo,FilingNo,Court,Petitioner,Respondent,RespondentOffice,OICId,OICMobileNo,CaseSubjectId,Remarks,HearingDate,CaseNo,OrderComplianceDate,IsComplaince,CaseSubSubjectId,IsOrderByDirection from tbl_OrderByDirectionPendingCase where CaseType='" + Convert.ToString(CaseType) + "' and IsOrderByDirection='Yes' and IsComplaince='Yes'");
