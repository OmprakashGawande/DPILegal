...
public partial class Legal_Order_By_Direction_Count_IsComplaincePending : System.Web.UI.Page
{
    DataSet dsCase = null;
    ...
    protected void BindGrid(string CaseType)
    {
        try
        {
            dsCase = obj.ByDataSet("select distinct UniqueNo,FilingNo,Court,Petitioner,Respondent,RespondentOffice,OICId,OICMobileNo,CaseSubjectId,Remarks,HearingDate,CaseNo,OrderComplianceDate,IsComplaince,CaseSubSubjectId,IsOrderByDirection from tbl_OrderByDirectionPendingCase where CaseType='" + Convert.ToString(CaseType) + "' and IsOrderByDirection='Yes' and IsComplaince='Pending'");
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            if ((e.Row.RowState & DataControlRowState.Edit) <= 0)
            {
                ...
                if (lblCaseSubSubjectId.Text != "0" && lblCaseSubSubjectId.Text != "")
                {
                    ...
                    if (dsCase.Tables.Count > 0 && dsCase.Tables[0].Rows.Count > 0 && !string.IsNullOrEmpty(dsCase.Tables[0].Rows[0]["CaseSubSubject"].ToString()) )
                    {
