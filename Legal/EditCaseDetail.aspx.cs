...
public partial class Legal_EditCaseDetail : System.Web.UI.Page
{
    ...
    #region DropDown Bind Event
    protected void FillHODName()
    {
        try
        {
            ddlNameHod.Items.Clear();
    ...
    protected void FillDitrict()
    {
        try
        {
            ddlDistrict.Items.Clear(); ddlOldOICDistrict.Items.Clear(); ddlPetitionerPresentDistrict.Items.Clear();
    ...
    protected void FillOicName()
    {
        try
        {
            ...
            DataSet dtOic = obj.ByDataSet("select distinct OICMaster_ID, OICName, OICMobileNo, OICEmailID from tblOICMaster OM inner join tbl_DistrictCourtMaping_Mst CD on CD.District_ID = OM.District_Id where OM.District_Id=" + ddlOICDisrtrict.SelectedValue);
    ...
    protected void FillDesignation()
    {
        try
        {
            ...
            ddlPetiDesigNation.Items.Clear(); ddlResDesig.Items.Clear(); ddlOICDesignation.Items.Clear();
    ...
    protected void FillCasetype()
    {
        try
        {
            ddlCasetype.Items.Clear(); ddloldCasetype.Items.Clear();
    ...
    protected void fillOICDistrict()
    {
        try
        {
            ddlOICDisrtrict.Items.Clear();
            DataSet dtOicDis = obj.ByDataSet("select distinct DM.District_ID, District_Name from  Mst_District DM inner join tbl_DistrictCourtMaping_Mst CMM on DM.District_ID = CMM.District_ID order by District_Name ASC");

            if (dtOicDis != null && dtOicDis.Tables[0].Rows.Count > 0)
            {
                ddlOICDisrtrict.DataTextField = "District_Name";
                ddlOICDisrtrict.DataValueField = "District_ID";
                ddlOICDisrtrict.DataSource = dtOicDis;
                ddlOICDisrtrict.DataBind();
            }
            ddlOICDisrtrict.Items.Insert(0, new ListItem("Select", "0"));
        }
        catch (Exception ex)
        {
            ErrorLogCls.SendErrorToText(ex);
                        ...
                        ds = obj.ByProcedure("USP_Update_CaseRegisDtl",
                             new string[] { "flag",
                                      "OICMaster_Id","OICOrderNumber","OICOrderDate","OICOrderDoc",
                                       "Case_ID","CaseReplyStatus","CaseReplyDate","CaseReplyRemark","OICDistrict","UniqueNo", "LastupdatedBy", "LastupdatedByIP" }
                           , new string[] { "1", ddlOicName.SelectedValue,txtOICcaseNumber.Text.Trim(),OICDate,OICDocument,
                            ViewState["ID"].ToString(),ddlCaseReply.SelectedValue,ReplyDate,txtReplyCaseRemark.Text.Trim(),ddlOICDisrtrict.SelectedValue,
                            ...
                            if (!string.IsNullOrEmpty(ds.Tables[1].Rows[0]["OICEmailID"].ToString()))
                            {
                                ...
                                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    using (MailMessage mm = new MailMessage(smtpSection.From, ds.Tables[1].Rows[0]["OICEmailID"].ToString()))
                                    {
                                        ...
                                        mm.Body = content;
                                        string checkDoc = ds.Tables[1].Rows[0]["OICOrderDoc"].ToString();
                                        if (!string.IsNullOrEmpty(checkDoc))
                                        {
                                            if (!checkDoc.Contains("NA"))
                                            {
    ...
    protected void btnPetitioner_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                dspeti = obj.ByProcedure("USP_InsertUpdate_PetiDtlForCaseRegi", new string[] { "flag", "UniqueNo", "Case_Id", "PetitionerName", "Designation_Id", "PetitionerMobileNo", "PetitionerAddress", "District_ID", "CreatedBy", "CreatedByIP", "PetitionerRemark" }
                    , new string[] { "1", ViewState["UniqueNO"].ToString(), ViewState["ID"].ToString(), txtPetiName.Text.Trim(), ddlPetiDesigNation.SelectedValue, txtPetiMobileNo.Text.Trim(), txtPetiAddRess.Text.Trim(), ddlPetitionerPresentDistrict.SelectedValue, ViewState["Emp_Id"].ToString(), obj.GetLocalIPAddress(), txtPetitionerRemark.Text.Trim() }, "dataset");
            }
            else if (btnPetitioner.Text == "Update")
            {
                dspeti = obj.ByProcedure("USP_InsertUpdate_PetiDtlForCaseRegi", new string[] { "flag", "UniqueNo", "Petitioner_Id", "PetitionerName", "Designation_Id", "PetitionerMobileNo", "PetitionerAddress", "District_ID", "LastupdatedBy", "LastupdatedByIP", "PetitionerRemark" }
                  , new string[] { "2", ViewState["UniqueNO"].ToString(), ViewState["Petitioner_ID"].ToString(), txtPetiName.Text.Trim(), ddlPetiDesigNation.SelectedValue, txtPetiMobileNo.Text.Trim(), txtPetiAddRess.Text.Trim(), ddlPetitionerPresentDistrict.SelectedValue, ViewState["Emp_Id"].ToString(), obj.GetLocalIPAddress(), txtPetitionerRemark.Text.Trim() }, "dataset");
            }
    ...
    protected void btnRespondent_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {   // Add New Column HOD_Id into Insert/Update operation by bhanu on 09/05/23
                dsres = obj.ByProcedure("USP_InsertUpdate_RespondentForCaseRegis", new string[] { "flag", "HOD_Id", "UniqueNo", "Case_Id", "Officetype_Id", "Office_Id", "Designation_Id", "RespondentName", "RespondentMobileNo", "Address", "Department", "Department_Id", "RespondentRemark", "CreatedBy", "CreatedByIP" }
                    , new string[] { "1", ddlHodName.SelectedValue, ViewState["UniqueNO"].ToString(), ViewState["ID"].ToString(), ddlResOfficetypeName.SelectedValue, ddlResOfficeName.SelectedValue, ddlResDesig.SelectedValue, txtResName.Text.Trim(), txtResMobileNo.Text.Trim(), txtResAddress.Text.Trim(), ddlResDepartment.SelectedItem.Text, ddlResDepartment.SelectedValue, txtRespondentRemark.Text.Trim(), ViewState["Emp_Id"].ToString(), obj.GetLocalIPAddress() }, "dataset");
            }
            else if (btnRespondent.Text == "Update")
            {
                dsres = obj.ByProcedure("USP_InsertUpdate_RespondentForCaseRegis", new string[] { "flag", "HOD_Id", "UniqueNo", "Respondent_ID", "Officetype_Id", "Office_Id", "Designation_Id", "RespondentName", "RespondentMobileNo", "Address", "Department", "Department_Id", "RespondentRemark", "LastupdatedBy", "LastupdatedByIP" }
    ...
    protected void btnUpdateCaseDtl_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (btnUpdateCaseDtl.Text == "Update")
            {
                ...
                "HighPriorityCase_Status", "LastupdatedBy", "LastupdatedByIP", "Case_ID", "UniqueNo","CaseDetail","PolicyMeterStatus","CaseRegDate" },
                    new string[] { "4", HODId,ddlCaseBelongto.SelectedValue, ddlDistrict.SelectedValue, ddlCaseSubject.SelectedValue,
                    ddlCaseSubSubject.SelectedValue, HinghPriorityStatus,ViewState["Emp_Id"].ToString(),obj.GetLocalIPAddress(),ViewState["ID"].ToString(),ViewState["UniqueNO"].ToString(),txtCaseDetail.Text.Trim(),PolicyMeterStatus,CaseRegDate}, "dataset");
            }
            if (Dsupdate.Tables.Count > 0)
            {
                if (Dsupdate.Tables[0].Rows.Count > 0)
                {
                    if (Dsupdate.Tables[0].Rows[0]["Msg"].ToString() == "OK")
                    {
                        ...
                        ddlCaseSubSubject.ClearSelection(); ddlHighprioritycase.ClearSelection(); txtCaseDetail.Text = ""; txtCaseRegistrationDate.Text = "";
                        BindDetails(sender, e);
                    }
    ...
    protected void btnIntrimOrder_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (btnIntrimOrder.Text == "Update")
            {
                dsintrim = obj.ByProcedure("USP_InserUpdateIntrimOrder", new string[] { "IntrimOrderStartDate", "IntrimOrderEndDate", "IntrimOrderTimeline", "IntrimOrderSummary", "IntrimOrderAnyPrevPP", "CreatedBy", "CreatedByIP", "Case_ID" },
                new string[] { IntrimOrderStartDate, IntrimOrderEndDate, txtIntrimTimeline.Text.Trim(), txtIntrimOrderSummary.Text.Trim(), txtIntrimPrevPP.Text.Trim(), ViewState["Emp_Id"].ToString(), obj.GetLocalIPAddress(), ViewState["ID"].ToString() }, "dataset");
            }
            ...
            if (Page.IsValid)
            {
                ...
                if (errormsg == "")
                {
                    ...
                    ds = obj.ByProcedure("USP_Insert_DistrictWiseReturnFileAndAppeal", new string[] { "Case_ID", "Action_ID", "ActionYesOrNo_ID", "ReceiptNo", "ReceiptDate", "ActionAppealedFrom",
                                                                                                      ...
                                                                                                      "FileMoment_ID","HOD_Section_ID","HOD_Section", "CreatedBy", "CreatedByIP" }
                                                    , new string[] { ViewState["ID"].ToString(), ddlAction.SelectedValue, ddlYesOrNo.SelectedValue, txtReceiptNo.Text.Trim(), ReceiptDate, txtActionAppealedFrom.Text.Trim(), ddlNameHod.SelectedValue,
                                                                     txtDispatchNo.Text.Trim(), DispatchDate, ViewState["ActionDoc"].ToString(), txtHODRemark.Text.Trim(), txtReason.Text.Trim()
                                                                      ,RDFileMoment.SelectedValue,ddlSection.SelectedValue,ddlSection.SelectedItem.Text,ViewState["Emp_Id"].ToString(), obj.GetLocalIPAddress() }, "dataset");
                    ...
                    if (ds != null)
                    {
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            if (ds.Tables[0].Rows[0]["Msg"].ToString() == "OK")
                            {
                                ...
                                GetReturnFileAppeal(sender, e); ddlSection.ClearSelection();
                                ClientScript.RegisterClientScriptBlock(this.GetType(), "alert", "swal('Alert!', '" + ErrMsg + "', 'success')", true);
                            }
                            else
                                ClientScript.RegisterClientScriptBlock(this.GetType(), "alert", "swal('Warning!','" + ErrMsg + "' , 'warning')", true);
        ...
        try
        {
            ...
            else
            {
                if (e.CommandName == "EditRecord")
                {
                    ...
                    Label lblPresentpostingAddress = (Label)row.FindControl("lblPresentpostingAddress");
                    ...
                    txtPetiAddRess.Text = !string.IsNullOrEmpty(lblPresentpostingAddress.Text) ? lblPresentpostingAddress.Text : null;
    ...
    protected void GrdRespondentDtl_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            {
                GridViewRow row = (GridViewRow)(((LinkButton)e.CommandSource).NamingContainer);
                DataSet disable = obj.ByProcedure("Usp_DeleteRespondend", new string[] { "UniqueNo", "Respondent_ID", "LastIsactiveBy", "LastIsactiveByIP" }
                   , new string[] { ViewState["UniqueNO"].ToString(), e.CommandArgument.ToString(), ViewState["Emp_Id"].ToString(), obj.GetLocalIPAddress() }, "dataset");
                if (disable != null && disable.Tables[0].Rows.Count > 0)
                {
                    string ErrMsg = disable.Tables[0].Rows[0]["ErrMsg"].ToString();
                    if (disable.Tables[0].Rows[0]["Msg"].ToString() == "OK")
    ...
    protected void GetReturnFileAppeal(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(ds.Tables[0].Rows[0]["HOD_Section_ID"].ToString()))
                {
                    ddlNameHod_SelectedIndexChanged(sender, e);
                    ddlSection.ClearSelection();
                    ddlSection.Items.FindByText(ds.Tables[0].Rows[0]["HOD_Section_ID"].ToString().Trim()).Selected = true;
                }
            }
        }
        catch (Exception ex)
    ...
    #region DropDown Selected Event
    ...
    protected void ddlIntrimOrder_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            lblMsg.Text = "";
            if (ddlIntrimOrder.SelectedValue == "1")
            {
                Div_IntrimOrderYesNO.Visible = true;
            }
            else if (ddlIntrimOrder.SelectedValue == "2")
            {
                Div_IntrimOrderYesNO.Visible = true;
                txtIntirmOrderDate.Text = "";
                txtIntrimTimeline.Text = "";
                txtIntrimPrevPP.Text = "";
                txtIntrimOrderEnddate.Text = "";
                txtIntrimOrderSummary.Text = "";
                rfvIntrimODate.Enabled = false;
                rfvOTimeline.Enabled = false;
                rfvintrimPP.Enabled = false;
                RequiredFieldValidator1.Enabled = false;
                rfvIOS.Enabled = false;

                btnIntrimOrder.Text = "Update";
            }
            else
            {
                Div_IntrimOrderYesNO.Visible = false;
            }
        }
        catch (Exception ex)
        {
    ...
    protected void ddlNameHod_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddlNameHod.SelectedValue == "2")
            {
                div_HODSection.Visible = true;
                ddlSection.ClearSelection();
            }
            else
                div_HODSection.Visible = false;
            ddlSection.ClearSelection();
        }
        catch (Exception ex)
        {
    ...
    protected void ddlOICDisrtrict_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            FillOicName();
    ...
    protected void RDFileMoment_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                divReturnAndReply.Visible = true; div_HODSection.Visible = false;
            ...
            else
            {
                ...
                txtHODRemark.Text = ""; div_HODSection.Visible = false;
            }
        }
        catch (Exception ex)
        {
    ...
    protected void BindDetails(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(PolicyMeter))
                {
                    ddlPolicyMeter.ClearSelection();
                    ddlPolicyMeter.Items.FindByText(PolicyMeter).Selected = true;
                }
                ...
                if (ddlCourtType.Items.Count > 0)
                    ddlCourtType.Items.FindByValue(ds.Tables[0].Rows[0]["CourtType_Id"].ToString().Trim()).Selected = true;
                ddlCourtType.Enabled = false;
                ...
                if (!string.IsNullOrEmpty(ds.Tables[0].Rows[0]["CaseSubSubj_Id"].ToString()))
                {
                    ...
                    int SubSubject = ReutrnSubSubjectID().AsEnumerable().Where(r => r.Field<int>("CaseSubSubj_Id") == Convert.ToInt32(ds.Tables[0].Rows[0]["CaseSubSubj_Id"])).Select(y => y.Field<int>("CaseSubSubj_Id")).FirstOrDefault();
                    if (SubSubject != 0)
                    {
                        if (SubSubject == Convert.ToInt32(ds.Tables[0].Rows[0]["CaseSubSubj_Id"]))
                        {
                            ddlCaseSubject_SelectedIndexChanged(sender, e);
                            ddlCaseSubSubject.ClearSelection();
                ...
                if (!string.IsNullOrEmpty(ds.Tables[0].Rows[0]["OICDistrict"].ToString()))
                {
                    ddlOICDisrtrict.ClearSelection();
                    //fillOICDistrict();
                    ddlOICDisrtrict.Items.FindByValue(ds.Tables[0].Rows[0]["OICDistrict"].ToString().Trim()).Selected = true;
                }
                if (!string.IsNullOrEmpty(ds.Tables[0].Rows[0]["OICMaster_Id"].ToString()))
                {
                    ddlOicName.Items.Clear();
                    ddlOICDisrtrict_SelectedIndexChanged(sender, e);
                    int OICId = ReutrnOicName().AsEnumerable().Where(r => r.Field<int>("OICMaster_ID") == Convert.ToInt32(ds.Tables[0].Rows[0]["OICMaster_Id"])).Select(y => y.Field<int>("OICMaster_ID")).FirstOrDefault();
                    if (OICId != 0)
                    {
                        if (OICId == Convert.ToInt32(ds.Tables[0].Rows[0]["OICMaster_Id"]))
                        {
                            ddlOICDisrtrict_SelectedIndexChanged(sender, e);
                            ddlOicName.ClearSelection();
                            ...
                            ddlCaserRelated.ClearSelection();
    ...
    public DataTable ReutrnOicName()
    {
        try
        {
            Helper oic = new Helper();
            ...
            //   DataTable dtOic = oic.GetOIC(ddlCourtType.SelectedValue) as DataTable;
            DataSet dtOic = obj.ByDataSet("select OICMaster_ID, OICName, OICMobileNo, OICEmailID from tblOICMaster OM inner join tbl_DistrictCourtMaping_Mst CD on CD.District_ID = OM.District_Id where OM.District_Id=" + ddlOICDisrtrict.SelectedValue);
